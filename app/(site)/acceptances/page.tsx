...
import { getAcceptances, getPageContent } from '@/lib/data'
import { section } from '@/lib/utils'
import { GraduationCap } from 'lucide-react'

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageContent('acceptances')
  return {
    title: page?.metaTitle ?? 'Acceptances Wall — University Offers Secured by EduCoach Students',
    description: page?.metaDescription ?? 'Browse the EduCoach Acceptances Wall — a visual showcase of university offers and scholarships secured by our students across 30+ countries.',
  }
}

export default async function AcceptancesPage() {
  const [acceptances, page] = await Promise.all([
    getAcceptances(),
    getPageContent('acceptances'),
  ])
    ...
    <>
      ...
      <section className="bg-secondary/40">
        <div className="mx-auto max-w-4xl px-4 py-16 text-center md:py-24">
          <SectionHeading
            eyebrow={s('join').eyebrow ?? 'Join Them'}
            title={s('join').title ?? 'See your name on this wall'}
            description={s('join').description ?? 'Book a free assessment and take the first step toward your dream university.'}
          />
          <div className="mt-8">

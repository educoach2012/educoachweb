...
import { getCountries, getServices, getSiteSettings, getPageContent } from "@/lib/data"

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageContent('book-assessment')
  return {
    title: page?.metaTitle ?? 'Book Your Free Assessment',
    description: page?.metaDescription ?? 'Book a free, no-obligation study abroad assessment with EduCoach Services. Get a personalised roadmap for your admission, scholarship and visa journey.',
  }
}

const defaultPerks = [
  { icon: "compass", title: "Personalised roadmap", desc: "A step-by-step plan mapped to your profile, budget and target intake." },
  { icon: "award", title: "Scholarship review", desc: "We surface funding you actually qualify for across 500+ universities." },
  { icon: "shield", title: "Visa confidence", desc: "Documentation and interview prep from a 98% approval-rate team." },
  { icon: "clock", title: "30-minute call", desc: "A focused session with a senior counsellor — no sales pressure." },
]

export default async function BookAssessmentPage() {
  const [countries, services, settings, page] = await Promise.all([
    getCountries(),
    getServices(),
    getSiteSettings() as Promise<{ assessmentFormUrl?: string | null }>,
    getPageContent('book-assessment'),
  ])

  const perks = (page?.perks ?? defaultPerks) as Array<{ title: string; desc: string; icon?: string }>
                  ...
                  <li key={perk.title}>
                    <Reveal delay={i * 0.08} className="flex gap-4">
                      <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-secondary/15 text-secondary-foreground">
                        <Icon name={perk.icon ?? 'Lightbulb'} className="size-5 text-primary" />
                      </span>
                      <div>

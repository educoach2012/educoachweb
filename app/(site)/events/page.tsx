...
export const metadata: Metadata = {
  title: 'Events',
  description:
    'Join EduCoach\'s free masterclasses, workshops and webinars on admissions, scholarships and country-specific strategies.',
}
...
export default async function EventsPage() {
  ...
  return (
    <>
      ...
      <PageHero
        eyebrow="Events"
        title="Free masterclasses & workshops"
        description="Join our live sessions on admissions, scholarships and country-specific strategies. All sessions are free."
        crumbs={[{ label: 'Events' }]}
      />
      ...
      {upcoming.length > 0 && (
        <section className="py-16 md:py-24">
          <div className="mx-auto max-w-7xl px-4">
            <SectionHeading
              eyebrow="Upcoming"
              title="Register for an upcoming session"
              description="Limited seats — reserve yours today."
            />
        ...
        <section className="border-t border-border bg-secondary/40 py-16 md:py-24">
          <div className="mx-auto max-w-7xl px-4">
            <SectionHeading
              ...
              description="Missed these? Follow us to catch the next one."
            />
            <RevealStagger className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {past.map((e) => (
                <RevealItem key={e.title}>
      ...
      <CTABand
        title="Can't attend an event? Book a 1-on-1 instead."
        subtitle="Get personalised guidance from a senior counsellor — at a time that suits you."
      />
    </>
  )
}
